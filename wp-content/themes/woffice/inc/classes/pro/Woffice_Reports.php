...
if( ! class_exists( 'Woffice_Reports' ) ) {
    class Woffice_Reports
    {
         ...
         * @return array
         */
        protected function provide_data() {
            ...
            $table_name_polls = $wpdb->prefix . 'woffice_poll';
            $data['number_polls'] = $wpdb->get_results("SELECT COUNT(*) AS number_polls FROM ".$table_name_polls);
            $data['polls'] = $wpdb->get_results("SELECT * FROM $table_name_polls ORDER BY id DESC LIMIT 5");
            ...
            foreach($bp_activities['activities'] as $key=>$activity) {
                ...
                // We only keep the 10 most recent activities
                if ($key == 9) break;
            }
        ...
        public function hub_get_data() {
             ...
             */
            $email = base64_decode($_SERVER['HTTP_X_EMAIL']);
            if(empty($email)) {
                echo json_encode([
                    'type' => 'error',
             ...
             */
            $product_key = base64_decode($_SERVER['HTTP_X_PRODUCTKEY']);
            ...
            $provided_data = $this->provide_data();
        ...
        public function hub_send_product() {
           ...
           */
            $pro_key = base64_decode($_SERVER['HTTP_X_PROKEY']);
            if(empty($pro_key)) {
                echo json_encode([
                    'type' => 'error',
             ...
             */
            if($pro_key !== get_option('alka_pro_key', false)) {
                echo json_encode([
                    'type' => 'error',
            ...
            $data = array(
                'product_key' => base64_encode(Woffice_Pro::$product_key)
            );
