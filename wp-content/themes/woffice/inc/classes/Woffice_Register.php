    ...
    class Woffice_Register
    {
        ...
        public function register_redirect(){
            ...
            if (isset($_POST['user_id']) && $_POST['user_id'] > 0) {
                ...
                if( $user ) {
                    ...
                    // Ajax content :
                    echo '<div class="woffice-ajax-main"><i class="fa fa-check-circle-o"></i> '.__('Redirecting', 'woffice').'...</div>';
                }
            }
            wp_die();
        ...
        public function registration_form()
        {
            ...
            <form method="post" action="<?php echo esc_url($_SERVER['REQUEST_URI']) . '#register-form'; ?>"
                ...
                <div class="login-form">
                    ...
                    <div class="form-group">
                        <input name="reg_password_confirmation" type="password" class="login-field"
                            ...
                            <?php if (bp_is_active('xprofile')) : if (bp_has_profile(array('profile_group_id' => 1, 'fetch_field_data' => false))) : while (bp_profile_groups()) : bp_the_profile_group(); ?>
                                <?php while (bp_profile_fields()) : bp_the_profile_field(); ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        if ( 'field_1' == bp_get_the_profile_field_input_name())
                                            continue;
        ...
        public function registration()
        {
            ...
            if (is_wp_error($this->validation())) {
                ...
                /* Temporary removed:
                    It should be fired when users verify their email, not when they sign up. It created many troubles
                    with the email messages involved with the Sign Up. It has also created a compatibility issue with
                    Eonet MUA.
                */
                ...
                if (!is_wp_error($register_user)) {
                    ...
                    if(woffice_is_enabled_confirmation_email()) {
                        ...
                        if ( apply_filters( 'bp_core_signup_send_activation_key', true, $user_id, $userdata['user_email'], $activation_key, array() ) ) {
                            bp_core_signup_send_validation_email( $user_id, $userdata['user_email'], $activation_key, $userdata['user_login'] );
                        }
                    }

                    $color_notifications_green = (function_exists('fw_get_db_settings_option')) ? fw_get_db_settings_option('color_notifications_green') : '';
                    echo '<div id="success-register" class="infobox fa-check-circle-o" style="background-color: ' . $color_notifications_green . ';" data-user="'.$user_id.'">';
                    if( (class_exists('pw_new_user_approve') || function_exists('eonet_manual_user_approve')) && !woffice_is_enabled_confirmation_email() ) {
                        $succesful_message = '<strong>' . __('Registration complete. Please wait for approval. ', 'woffice') . '</strong>';
                    } else if(woffice_is_enabled_confirmation_email()) {
	                    $succesful_message = '<strong>' . __('Registration complete. We sent you a confirmation email to:', 'woffice') . ' ' . $userdata['user_email'] . '</strong>';
                    } else {
	                    $succesful_message = '<strong>' . __('Registration complete. You can now ', 'woffice') . ' <a href="' . wp_login_url() . '">' . __('Sign In', 'woffice') . '</a></strong>';
                    }
                     ...
                     */
                    echo apply_filters('woffice_registration_completed_message', $succesful_message);
        ...
        public function send_new_user_notifications( $user_id ) {
            ...
            if( $login_custom != 'yep' || woffice_is_enabled_confirmation_email() || function_exists('eonet_manual_user_approve'))
                return;
