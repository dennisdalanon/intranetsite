    ...
    class Woffice_Frontend
    {
        ...
        static function frontend_process( $type, $post_id = 0, $is_shortcode = false ) {
            ...
            if ( empty($errors) ) {
                 ...
                 * @param WP_Post
                 */
                do_action('woffice_after_{$type}_{$process_type}', get_post($post_id));

                if($is_shortcode == false && $frontend_state == 'publish') {
                    /**
                     * Alerts
                     */
                    $message = ucfirst($type). ' '.$process_type.'!';
        ...
        static function frontend_render( $type, $process_val, $post_id = 0 ) {
            ...
            if(is_int($process_val) && $process_val != 0){
                $html .= '<div class="infobox fa fa-check-circle-o" style="background-color: #3cb500;">';
             ...
             */
            $html .= '<div id="' . $type . '-'.$form_type.'" class="intern-padding">';

            $form_extra = ( $type == "blog" || $type == "directory" ) ? ' enctype="multipart/form-data"' : '';

            $html .= '<form action="'. self::get_form_url($type) .'" id="primaryPostForm" method="POST" '.$form_extra.'>';
            ...
            if ( $type == 'project' ) {
                ...
                $author_id = ( !is_null( $post)) ? $post->post_author : null;
                ...
                if(apply_filters('woffice_frontend_project_tracking_enabled', true)) {
                  ...
                  $html .= '<select name="tracking_type" class="form-control">';
...
//		                $members_included = apply_filters( 'woffice_project_assignation_include', '', get_the_ID(), $project_members );
//		                $members_included = (is_array($members_included)) ? implode(',', $members_included) : $members_included;
//
//		                $members_excluded = apply_filters( 'woffice_project_assignation_exclude', $project_members_value . '', get_the_ID(), $project_members );
//		                $members_excluded = (is_array($members_excluded)) ? implode(',', $members_excluded) : $members_excluded;
		                ...
		                $html .= '<div class="form-group">';
			                ...
			                $html .= '<div class="woffice-users-suggest"
			                    ...
			                    >';
		                        //data-members-included="' . $members_included . '"
			                    //data-members-excluded="' . $members_excluded . '"
                ...
                if(class_exists('EventON') && $calendar_sync_enabled) {
                 ...
                 * @param bool
                 */
                $files_upload_enaled = apply_filters( 'woffice_allow_to_add_files_tab_on_project_creation', true );

                if(defined('fileaway') && $files_upload_enaled){
                ...
                $html .= '<div class="row">';
                    $html .= '<div class="col-md-6">';
                        $html .= '<p>';
                            ...
                            $html .= '<input type="text" name="item_location_lng" id="item_location_lng" placeholder="-88.242188"/>';
                        $html .= '</p>';
                    $html .= '</div>';
                    $html .= '<div class="col-md-6">';
                        $html .= '<p>';
                            ...
                            $html .= '<small>'. __( 'You can use : ', 'woffice' ) .'<a href="http://www.latlong.net/" target="_blank">LatLong.net</a></small>';
                            $html .= '<input type="text" name="item_location_lat" id="item_location_lat" placeholder="37.544577"/>';
                        $html .= '</p>';
                    $html .= '</div>';
                $html .= '</div>';
                ...
                $html .= '<div class="row">';
                    $html .= '<div class="col-md-6">';
                        $html .= '<p>';
                            ...
                            $html .= '<input type="text" name="item_button_text" id="item_button_text"/>';
                        $html .= '</p>';
                    $html .= '</div>';
                    $html .= '<div class="col-md-6">';
                        $html .= '<p>';
                            $html .= '<label for="item_button_icon">'. __( 'Button\ Icon (Font Awesome) :', 'woffice' ) .'</label>';
                            ...
                            $html .= '<input type="text" name="item_button_icon" id="item_button_icon" placeholder="fa-star"/>';
                        $html .= '</p>';
                    $html .= '</div>';
                    $html .= '<div class="col-md-12">';
                        $html .= '<p>';
                        $html .= '<label for="item_button_link">'. __( 'Button Link :', 'woffice' ) .'</label>';
                        $html .= '<input type="text" name="item_button_link" id="item_button_link"/>';
                        $html .= '</p>';
                    $html .= '</div>';
                $html .= '</div>';
            ...
            if ( $type != 'wiki' && $type != 'project' ) {
                ...
                $html .= '<p>';
                    ...
                    $html .= '<input type="file" name="post_thumbnail" id="post_thumbnail">';
                $html .= '</p>';
            ...
            $html .= '<p class="text-right">';
                ...
                $html .= '<button type="submit" class="btn btn-default">';
                    $html .= '<i class="fa fa-pencil"></i>'.$button_type .' '. $base_label;
                $html .= '</button>';
            $html .= '</p>';
             ...
             * Only for creation
             */
            if(empty($post_id) || $type == 'blog') {
                $html .= '<div class="center"><a href="javascript:void(0)" class="btn btn-default" id="hide-' . $type . '-'.$form_type.'">';
	     ...
	     * @return string
	     */
	    static function get_taxonomy_options($taxonomy, $post_terms, $parent_id, $tab) {

		    $terms = get_terms( $taxonomy, array( 'hide_empty' => false, 'child_of' => $parent_id ) );
		    ...
		    foreach ( $terms as $term ) {
			    ...
			    $html .= static::get_taxonomy_options( $taxonomy, $post_terms, $term->term_id, '> '.$tab);
		    }
