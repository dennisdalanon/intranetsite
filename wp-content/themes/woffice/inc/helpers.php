    ...
    function woffice_alerts_render() {
        ...
        <div id="woffice-alerts-wrapper">
            ...
            <?php foreach ($woffice_alerts as $alert) : ?>
                ...
                <div id="woffice-alert-<?php echo sanitize_html_class($alert->id); ?>" class="woffice-main-alert clearfix woffice-alert-<?php echo sanitize_html_class($alert->type); ?>">

                    <div class="container">

                        <p><i class="fa <?php echo esc_attr($icon_class); ?>"></i> <?php echo $alert->content; ?></p>
                        <a href="javascript:void(0)" class="woffice-alert-close pull-right"><i class="fa fa-close"></i></a>
...
if(!function_exists('woffice_title')) {
    ...
    function woffice_title($title)
    {
        ...
        // Get BuddyPress and the current post info
        global $bp;
        global $post;
        ...
        // Get the members & groups slugs from the settings
         ...
         * @param bool
         */
	      $map_enabled = apply_filters( 'woffice_directory_page_map_enabled',
                  (is_page_template("page-templates/page-directory.php") || is_tax('directory-category'))
                  && $is_user_allowed
                  && empty($slider_featured)
          );
        ...
        if (empty($slider_featured) || (!shortcode_exists('rev_slider') && !class_exists('FW_Extension_Slider'))) {
	        ...
	        if ( $hastitle != true ) {
		        ...
		        if ( is_singular( 'post' ) ) {
			        ...
			        if ( function_exists( 'bp_is_active' ) && bp_is_user() ) {
			        // $title = ( defined( 'bp_displayed_user_mentionname' ) ) ? bp_get_displayed_user_mentionname() : bp_get_displayed_user_fullname();
            ...
            // Use the sidewide image set in Theem Settings
            ...
            // Use the default background image coming with the theme as placeholder
            if ( empty($final_image) )
	            $final_image = get_template_directory_uri() . '/images/1.jpg';
            ...
            $background_style = 'style="background-image: url(' . esc_url($final_image) .')";';
            echo '<div class="featured-background" ' . $background_style . '><div class="featured-layer"></div></div>';
...
if (!function_exists('woffice_get_sidebar_state')) {
    ...
    function woffice_get_sidebar_state()
    {
        ...
        if (function_exists('bp_is_active')) {
	        ...
	        $is_groups_page = ( bp_is_active( 'groups' ) && bp_is_groups_component() );
        }

        if ( $is_members_page || $is_groups_page ) {

	        // Display sidebar on BuddyPress page
	        $sidebar_buddypress = woffice_get_settings_option('sidebar_buddypress');
	        $sidebar_option = ( $sidebar_buddypress == "nope" ) ? "none" : "show";
	        ...
	        $sidebar_option = ( $sidebar_blog == "nope" ) ? "none" : "show";
	        ...
	        // If the sidebar is displayed, return if it opened or closed
	        $sidebar_default_state = woffice_get_settings_option('sidebar_state');
	        $sidebar_option = ($sidebar_default_state == "nope") ? "hide" : "show";
...
if(!function_exists('woffice_redirect_to_login')) {
    ...
    function woffice_redirect_to_login( $param = '', $disable_redirect_to = true ) {
        ...
        if ( ! $disable_redirect_to ) {
            ...
            //Get the previous url from the session if it is already stored - todo (Temporary removed)
            //$redirect_to = (isset($_SESSION['woffice_login_previous_url']) && !empty($_SESSION['woffice_login_previous_url'])) ? esc_url(urldecode($_SESSION['woffice_login_previous_url'])) : null;
	        ...
	        $_SESSION['woffice_login_previous_url'] = $redirect_to;
...
if(!function_exists('woffice_get_users_ids_by_xprofile_fields')) {
	...
	function woffice_get_users_ids_by_xprofile_fields( $fields ) {
		...
		if(isset($_POST['wordpress_email']) && !empty($_POST['wordpress_email'])) {
			...
			$ids_found_by_email = $users->get_results();//Add WHERE only one time

		}

		$all_ids_found = (array_unique(array_merge($ids_found_by_xprofile_fields, $ids_found_by_email), SORT_REGULAR));
...
if( !function_exists('woffice_get_redirect_page_after_login')) {
	...
	function woffice_get_redirect_page_after_login() {
		...
		if($aft_login == "custom") {
			...
			$previous_url = (isset($_SESSION['woffice_login_previous_url']) && !empty($_SESSION['woffice_login_previous_url'])) ? esc_url(urldecode($_SESSION['woffice_login_previous_url'])) : null;
...
if(!function_exists('woffice_action_theme_replace_gmaps_script')) {
    /**
     * Setting our own / customer key for the Google Map API
     */
    function woffice_action_theme_replace_gmaps_script() {
...
if ( !function_exists('eo_get_relative_current_buddypress_page_id')) {
	/**
	 ...
	 * @return bool|int
	 */
	function woffice_get_relative_current_buddypress_page_id() {
		...
		} else if (function_exists('bp_is_activity_directory') && bp_is_activity_directory() ) {
