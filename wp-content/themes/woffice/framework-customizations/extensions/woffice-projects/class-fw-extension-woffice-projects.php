...
class FW_Extension_Woffice_Projects extends FW_Extension {
	...
	public function woffice_projects_update_postmeta($the_ID,$new_todos) {
		...
		$page_builder = woffice_get_post_option($the_ID, 'page-builder', '');
		$project_date_start = woffice_get_post_option($the_ID, 'project_date_start', '');
		$project_date_end = woffice_get_post_option($the_ID, 'project_date_end', '');
        $project_archived = woffice_get_post_option($the_ID, 'project_completed', '');
		$project_members = woffice_get_post_option($the_ID, 'project_members', '');
		$project_links = woffice_get_post_option($the_ID, 'project_links', '');
		$project_edit = woffice_get_post_option($the_ID, 'project_edit', '');
        $only_author_can_edit = woffice_get_post_option($the_ID, 'only_author_can_edit', '');
		$single_project_visibility = woffice_get_post_option($the_ID, 'single_project_public', 'default');
		$project_files = woffice_get_post_option($the_ID, 'project_files', '');
	
		
		// We Update the Post Data
        $project_progress = woffice_get_post_option($the_ID, 'project_progress', '');
        $project_calendar = woffice_get_post_option($the_ID, 'project_calendar', '');
        $project_wunderlist = woffice_get_post_option($the_ID, 'project_wunderlist', '');
        $completion_date = woffice_get_post_option($the_ID, 'completion_date', 0);
		...
		$project_data = array(
            ...
            'single_project_public' => $single_project_visibility,
		...
		foreach ($project_todo_lists as $key=>$todo) {
			...
			if ($todo['email_sent'] == 'not_sent' && $todo['assigned'] != array('nope') && $todo['assigned'] != 'nope') {
				...
				foreach ($assigned_ready as $assigned) {
					 ...
					 * @param array $headers
					 */
					$headers = apply_filters('woffice_projects_assigned_email_heades', $headers);
