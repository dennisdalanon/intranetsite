				...
				if(is_array($dpProEventCalendar['custom_fields_counter'])) {
					...
					foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
						...
						if($field_value != "") {
							...
							$return .= '<div class="pec_event_page_custom_fields pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'">';
				...
				if(($this->calendar_obj->date_range_start != '0000-00-00' && $this->calendar_obj->date_range_start != NULL && (strtotime($curDate) < strtotime($this->calendar_obj->date_range_start)) || ( $this->calendar_obj->date_range_end != '0000-00-00' && $this->calendar_obj->date_range_end != NULL && strtotime($curDate) > strtotime($this->calendar_obj->date_range_end))) && !$this->is_admin) {
					...
					$html .= '
						<div class="dp_pec_date dp_pec_date_'.strtolower(date('l', strtotime($curDate))).' '.($countEvents > 0 && !$booked_date && $booking_remain ? 'pec_has_events' : '').' '.($general_count % 7 == 0 ? 'first-child' : '').' '.($special_date != "" ? 'dp_pec_special_date' : '').'" data-dppec-date="'.$curDate.'">
							<div class="dp_pec_date_item" '.$special_date.'><div class="dp_date_head"><span>'.$i.'</span></div>
							';
					...
					if($this->calendar_obj->show_titles_monthly && empty($this->event_id) && !$compact) {
						...
						foreach($eventsCurrDate as $event) {
							...
							$html .= '<a class="dp_daily_event" style="'.($event_color != "" ? 'background-color:'.$event_color.';' : '').($count_monthly_title >= $calendar_per_date ? 'display:none;' : '').'" data-dppec-event="'.$event->id.'" href="'.$href.'" '.($href != '#' ? 'target="'.$this->calendar_obj->link_post_target.'"' : '').' data-dppec-date="'.$curDate.'">'.$title.'</a>';
		...
		if($this->calendar_obj->daily_weekly_layout == 'schedule') {
			...
			for($i = $this->calendar_obj->limit_time_start; $i <= $this->calendar_obj->limit_time_end; $i++) {
				...
				foreach($result as $event) {
					...
					$html .= '<a class="dp_daily_event" '.($event_color != "" ? 'style="background-color:'.$event_color.'"' : '').' data-dppec-event="'.$event->id.'" href="'.$href.'" '.($href != '#' ? 'target="'.$this->calendar_obj->link_post_target.'"' : '').' data-dppec-date="'.$curDate.'">'.$title.'</a>';
				}
				$html .= '</div></div>
					</div>';
			}
		...
		for($x = 1; $x <= 7; $x++) {
			...
			$curDate = date('Y-m-d', $week_days[$x - 1]);
				...
				foreach($result as $event) {
					...
					$html_tmp = '<a class="dp_daily_event" '.($event_color != "" ? 'style="background-color:'.$event_color.'"' : '').' data-dppec-event="'.$event->id.'" href="'.$href.'" target="'.$this->calendar_obj->link_post_target.'" data-dppec-date="'.$curDate.'">'.$title.'</a>';
		...
		foreach($result as $event) {
			...
			if(is_array($dpProEventCalendar['custom_fields_counter']) && $this->type != 'compact') {
				...
				foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
					...
					if($field_value != "") {
						$html .= '<div class="pec_event_page_custom_fields pec_custom_'.$dpProEventCalendar['custom_fields']['id'][$counter].'"><strong>'.$dpProEventCalendar['custom_fields']['name'][$counter].'</strong>';
						if($dpProEventCalendar['custom_fields']['type'][$counter] == 'checkbox') { 
			...
			//if($this->limit_description > 0) {
				...
				if($excerpt != "") {
					...
					$event_desc_short = force_balance_tags(html_entity_decode(dpProEventCalendar_trim_words(htmlentities($event_desc), $this->limit_description)));
				}
				//}
		...
		if(count($events_obj) == 0) {
			...
			if(empty($order_events)) {
				...
				foreach($order_events as $event) {
					...
					if((is_numeric($this->columns) && $this->columns > 1) || $event->tbc || $this->opts['force_dates'] ) {
						if($event->tbc) {
							$html .= '<span class="dp_pec_date_time"><i class="fa fa-calendar-o"></i>'.$this->translation['TXT_TO_BE_CONFIRMED'].'</span>';
						} else {
							$html .= '<span class="dp_pec_date_time"><i class="fa fa-calendar-o"></i>'.$start_date.$end_date.'</span>';
					...
					if(is_array($dpProEventCalendar['custom_fields_counter'])) {
						...
						foreach($dpProEventCalendar['custom_fields_counter'] as $key) {
							if(!empty($cal_form_custom_fields) && $cal_form_custom_fields != "" && !in_array($dpProEventCalendar['custom_fields']['id'][$counter], $cal_form_custom_fields_arr)) {
								$counter++;
								continue;
							}
		...
		if($this->type == 'calendar') {
			...
			$html .= '
				<div class="dp_pec_nav dp_pec_nav_monthly" '.($this->calendar_obj->view == "monthly" || $this->is_admin || !empty($this->event_id) ? "" : "style='display:none;'").'>';
					$html .= '<span class="next_month"><strong>'.$this->translation['NEXT_MONTH'].'</strong> &raquo;</span>';
					$html .= '<span class="prev_month">&laquo; <strong>'.$this->translation['PREV_MONTH'].'</strong></span>';
			...
			$html .= '<span class="next_day"><strong>'.$this->translation['NEXT_DAY'].'</strong> &raquo;</span>';
			$html .= '<span class="prev_day">&laquo; <strong>'.$this->translation['PREV_DAY'].'</strong></span>';
			...
			$html .= '<span class="next_week"><strong>'.$this->translation['NEXT_WEEK'].'</strong> &raquo;</span>';
			$html .= '<span class="prev_week">&laquo; <strong>'.$this->translation['PREV_WEEK'].'</strong></span>';
