...
class EonetOptions {
	...
	static public function getSubmit($type = 'new', $label = '') {
		...
		if($type == 'delete') {
			$icon = 'fa-trash-o';
			$text = esc_html__('Delete', 'eonet-frontend-publisher');
		}
		elseif ($type == 'new') {
			$icon = 'fa-pencil-square-o';
			$text = esc_html__('New', 'eonet-frontend-publisher');
		} else {
			$icon = 'fa-check-circle';
			$text = esc_html__('Save', 'eonet-frontend-publisher');
		}
		...
		<div id="eo_field_<?php echo sanitize_html_class($option['name']); ?>" class="eo_field_iconpicker_wrapper">
			...
			<div class="eo_field_iconpicker_icons_details">
				<label class="eo_label_for_select">
					<span><?php esc_html_e('Icon groups:', 'eonet-frontend-publisher'); ?></span>
					<select class="eo_field_iconpicker_group_select">
	
						<option value="all" ><?php echo esc_html_x( 'All icons', 'Select option in the icon picker field', 'eonet-frontend-publisher' ) ?></option>
			...
			<select name="eo_field_<?php echo esc_attr($option['name']); ?>" class="eo_field_iconpicker_select">
				<option value="" <?php ( strcmp( '', $icon_value ) === 0 ? 'selected' : '' )?>><?php esc_html_e( 'No icon', 'eonet-frontend-publisher' ); ?></option>
				<?php
    ...
    static protected function renderFieldTypeUpload( $option, $option_saved ) {
        ...
        $html .= '<div class="eo_field_upload_wrapper">';
            ...
            // Button :
            $html .= '<button data-eo-name="eo_field_' . $option['name'] . '" name="eo_field_' . $option['name'] . '_trigger" class="eo_btn_upload_wp eo_btn eo_btn_default"><i class="fa fa-picture-o"></i> '.esc_html__('Choose an Image', 'eonet-frontend-publisher').'</button>';
        $html .= '</div>';
