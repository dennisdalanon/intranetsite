...
class EonetComponents{
    ...
    public function addSetting($links) {
        ...
        $link = '<a href="' . admin_url( 'plugins.php?page=eonet&eo_settings='.$this->current_component ) . '">'. esc_html__('Settings','eonet-frontend-publisher') .'</a>';
    ...
    public function ajaxStateChange() {
        ...
        if(!empty($method) && !empty($component)) {
            ...
            if($method == "deactivate") {
                ...
                $response['title'] = esc_html__('Extension has been deleted !', 'eonet-frontend-publisher');
                $response['content'] = esc_html__('The page will be refreshed in a few seconds...', 'eonet-frontend-publisher');
            }
            if($method == "activate") {
                $installed = $this->activate($component);
                if($installed == false){
                    $response['status'] = 'error';
                    $response['title'] = esc_html__('We couldn\'t install it...', 'eonet-frontend-publisher');
                    $response['content'] = esc_html__('Please try from your plugin page.', 'eonet-frontend-publisher');
                }
                /*$response['status'] = 'success';
                $response['title'] = esc_html__('We\'re installing the extension !', 'eonet-frontend-publisher');
                $response['content'] = esc_html__('The page will be refreshed in a few seconds...', 'eonet-frontend-publisher');*/
            }
        } else {
            $response['status'] = 'error';
            $response['title'] = esc_html__('Something went wrong...', 'eonet-frontend-publisher');
            $response['content'] = __('We\'ve not been able to reset your settings, please try again later.', 'eonet-frontend-publisher');
        }
    ...
    static public function getComponents($slug = '') {
        ...
        $components['live-search'] = array(
            'name' => esc_html__('Live Search', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-search',
            'premium' => false,
            'description' => esc_html__('Enhance your Wordpress search bar with customizable live search through all your post types and members.', 'eonet-frontend-publisher'),
        );

        $components['frontend-publisher'] = array(
            'name' => esc_html__('Frontend Publisher', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-compose-outline',
            'premium' => false,
            'description' => esc_html__('Allow yourself, your team or your user to edit and publish any Wordpress post or page.', 'eonet-frontend-publisher'),
        );

	    $components['manual-user-approve'] = array(
		    'name' => esc_html__('Manual User Approve', 'eonet-frontend-publisher'),
		    'version' => '1.0.0',
		    'icon' => 'ion-person-add',
		    'premium' => false,
		    'description' => esc_html__('Allows the admin of the site to approve or deny users manually, after they have registered.', 'eonet-frontend-publisher'),
	    );

        $components['live-notifications'] = array(
            'name' => esc_html__('Live Notifications', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-ios-loop',
            'premium' => false,
            'description' => esc_html__('Enables live notifications for all your users to get better interactions within your site.', 'eonet-frontend-publisher'),
        );

        $components['project-manager'] = array(
            'name' => esc_html__('Project Manager', 'eonet-frontend-publisher'),
            'version' => '1.0.0',
            'icon' => 'ion-briefcase',
            'premium' => false,
            'description' => esc_html__('Enables a complete project management tool for your site.', 'eonet-frontend-publisher'),
        );

        $components['social-connect'] = array(
            'name' => esc_html__('Social Connect', 'eonet-frontend-publisher'),
            'version' => '0.0.0',
            'icon' => 'ion-ios-cloud-download-outline',
            'premium' => false,
            'description' => esc_html__('Allow your visitor to login or register using Facebook, Twitter, Slack or Linkedin.', 'eonet-frontend-publisher'),
        );
	...
	public function load_textdomain() {
		...
		load_plugin_textdomain( 'eonet-frontend-publisher');
